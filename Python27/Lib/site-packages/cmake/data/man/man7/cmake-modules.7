...
.
.TH "CMAKE-MODULES" "7" "Jul 18, 2017" "3.9.0" "CMake"
.SH NAME
...
Languages \fBC\fP and \fBCXX\fP are supported.
.UNINDENT
.UNINDENT
.sp
...
the original \fBfind_package()\fP call.  It also sets an informative
diagnostic message if the dependency could not be found.
.sp
Any additional arguments specified are forwarded to \fBfind_package()\fP\&.
...
but if you don\(aqt use a default path for installation of the \fI\%QtIFW\fP tools,
...
.TP
.B DOXYGEN_SKIP_DOT
This variable has no any effect for component form of \fBfind_package\fP\&.
...
Variable indicating the MPI settings for \fB<lang>\fP were found.
.TP
.B \fBMPI_<lang>_COMPILER\fP
MPI Compiler wrapper for \fB<lang>\fP\&.
.TP
.B \fBMPI_<lang>_COMPILE_FLAGS\fP
Compilation flags for MPI programs, separated by spaces.
This is \fInot\fP a ;\-list\&.
.TP
...
.SS Usage
.sp
To use this module, call \fBfind_package(MPI)\fP\&.  If you are happy with the
auto\-detected configuration for your language, then you\(aqre done.  If
not, you have two options:
.INDENT 0.0
.IP 1. 3
Set \fBMPI_<lang>_COMPILER\fP to the MPI wrapper (e.g. \fBmpicc\fP) of your
choice and reconfigure.  FindMPI will attempt to determine all the
necessary variables using \fIthat\fP compiler\(aqs compile and link flags.
.IP 2. 3
If this fails, or if your MPI implementation does not come with
a compiler wrapper, then set both \fBMPI_<lang>_LIBRARIES\fP and
\fBMPI_<lang>_INCLUDE_PATH\fP\&.  You may also set any other variables
listed above, but these two are required.  This will circumvent
autodetection entirely.
.UNINDENT
.sp
When configuration is successful, \fBMPI_<lang>_COMPILER\fP will be set to
the compiler wrapper for \fB<lang>\fP, if it was found. \fBMPI_<lang>_FOUND\fP
and other variables above will be set if any MPI implementation was
found for \fB<lang>\fP, regardless of whether a compiler was found.
.sp
...
.UNINDENT
.sp
In new projects, please use the \fBOpenMP_<lang>_XXX\fP equivalents.
...
Allows the specified \fBprefix\fP to be prepended to the name of each
discovered test case. This can be useful when the same source files are
being used in multiple calls to \fBgtest_add_test()\fP but with different
\fBEXTRA_ARGS\fP\&.
.TP
.B \fBTEST_SUFFIX suffix\fP
Similar to \fBTEST_PREFIX\fP except the \fBsuffix\fP is appended to the name of
every discovered test case. Both \fBTEST_PREFIX\fP and \fBTEST_SUFFIX\fP can be
specified.
.TP
...
targets to the given \fBDESTINATION\fP\&. Its function is similar to that of
\fBinstall(EXPORTS ...)\fP\&.
.INDENT 0.0
.INDENT 3.5
.sp
.nf
...
.UNINDENT
.sp
This command writes a target export file \fB<filename>\fP for the named jar
...
(As of CMake version 3.9.0 there is only one API version.)
.sp
